...
class GridViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun editTodo(todo: TodoItem, title: String, notes: String, priority: Priority, dueDate: Long?) = viewModelScope.launch {
        repo.updateTodo(todo.copy(title = title, notes = notes, priority = priority, dueDate = dueDate))
    }
    ...
    fun editLog(log: LogEntry, content: String) = viewModelScope.launch {
        repo.updateLog(log.copy(content = content))
    }
